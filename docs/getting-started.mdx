...
Each Sale System lane is authenticated and authorised with a unique [SaleID](/docs/api-reference/data-model#saleid) + [KEK](/docs/api-reference/data-model#kek) combination.

Each POI Terminal is identified by a unique [POIID](/docs/api-reference/data-model#poiid). 

DataMesh associates [SaleID](/docs/api-reference/data-model#saleid) and [POIID](/docs/api-reference/data-model#poiid) values on Unify to enforce access restrictions between specific Sale System lanes and terminals.
...
DataMesh will provide the correct number of [SaleID](/docs/api-reference/data-model#saleid), [KEK](/docs/api-reference/data-model#kek), and [POIID](/docs/api-reference/data-model#poiid) values and terminals, and ensure the correct associations are implemented on Unify. 
...
1. Merchant closes the Sale System.
   - If the Sale System doesn't receive a payment response from Fusion App it needs to enter error handling by calling (GET /payments/{*SessionId*})
...
The Sale System must ensure that it can generate a QR code that can be scanned by the terminal to pair with the Sale System.

Depending on your integration type, your Sale System may be required to implement further changes to generate the pairing QR code.

Integration Type  | Changes required |
-----------------                         | :----:  |
Fusion App | [Already Supported](/docs/api-reference/fusion-app#configure-fusion-app)|
Fusion Cloud | ✔ |
Fusion Satellite | ❌ |
...
The properties listed below are represented as single characters in the pairing data JSON:

Property | QR | Format |
---------| ------ | ----------- |
[SaleID](/docs/api-reference/data-model#saleid) | s | String. Random. UUID v4. |
| | | 8-4-4-4-12 format. | 
PairingPOIID | p | String. Random. UUID v4. |
| | | 8-4-4-4-12 format. | 
[KEK](/docs/api-reference/data-model#kek) | k | String. Random. |
| | | 48-character string populated with characters 0-9, and A-F. |
| | | Please refer to the [sample KEK generation](#sample-kek-generation) |
[CertificationCode](/docs/api-reference/data-model#certificationcode) | c |  String. |
| | | Certification code provided by DataMesh. |
POSName | n |  String. |
| | | POS display name with at most 30 characters. |
| | | This can be the Work Station name or any name that you prefer. |
| | | Name displayed on the report and on the Satellite app (e.g. during a POS payment transaction). |
 Version | v | Numeric. |
| | | Pairing version code. Set to 1. |
...
Depending on your development language, there are available options and references that you can use to generate the pairing QR Code.
...
##### Scanning the QR code on the DataMesh terminal

Use these [instructions](/docs/appendix#qr-pos-pairing) to scan the pairing QR code on the DataMesh terminal.

##### LoginRequest After Pairing

Once pairing the Sale System with the terminal is successful, the Sale System must send an initial LoginRequest:
...
:::info
This functionality must still be implemented even if your Sale System supports [QR POS Pairing](#qr-pos-pairing) to cover unwanted scenarios wherein QR POS Pairing is not possible.
:::

The [SaleID](/docs/api-reference/data-model#saleid), [KEK](/docs/api-reference/data-model#kek), and [POIID](/docs/api-reference/data-model#poiid) are cashier configurable settings which may be different for each lane.
...
The DataMesh Fuel API is an extension to the core API which adds support for current and future fuel payment types accepted by DataMesh. (e.g. BP Card, BP Gift Card, FleetCard, Shell card)

#### Supported transaction types
* Purchase 
  * POS must handle dynamic surcharge 
  * Split-payment by item
* Refund
* Cashout is not supported on fuel cards

#### Purchase 

* A purchase request requires a populated the SaleItem[] array (for a full description see [SaleItem](https://datameshgroup.github.io/fusion/docs/api-reference/data-model#saleitem)). 
  * Fuel SaleItem is outlined below
* Sum of SaleItem[].ItemAmount must equal RequestedAmount 
* The purchase response will contain the fuel card used in [PaymentBrand](#paymentbrand)
...
* Must be a matched refund
* SaleItem[] must include items to refund
