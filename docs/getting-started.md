...
**[Fusion Satellite](./APIs/satellite-api-reference)**
...
**[Fusion Cloud](./APIs/cloud-api-reference)**
...
-----------------                         | :----:  | :------: | :------: |
Implement the [payment lifecycle](#payment-lifecycle)             | ✔ | ✔ | ✔ |
...
- [Fusion Satellite API](./APIs/satellite-api-reference)
...
- [Fusion Cloud API](./APIs/cloud-api-reference)
...
1. Sale system constructs a [payment request](./APIs/cloud-api-reference#paymentrequest) object.
1. Sale System sends the [payment request](./APIs/cloud-api-reference#paymentrequest) for the *payment-id* to the Fusion App's local HTTP end point for payments and waits for a response.
...
1. Sale System has the option to [link](#terminal-linking) with a POI Terminal by performing a [login request](./APIs/cloud-api-reference#error-handling) either: 
...
1. Merchant closes the Sale System.
1. Sale System can optionally send a [logout request](./APIs/cloud-api-reference#logout).
...
Before a payment is processed, the Sale System must be 'linked' to a POI terminal by sending a [login](./APIs/cloud-api-reference#login) request. 
...
Supported [payment](./APIs/cloud-api-reference#payment) types are:
...
The [ServiceID](./data-dictionary#serviceid) is also used by the Sale System in the [abort transaction request](./APIs/cloud-api-reference#abort-transaction) to cancel a specific payment transaction.  This means that the exact [ServiceID](./data-dictionary#serviceid) value in the payment request must be provided as the [ServiceID](./data-dictionary#serviceid) in the  [abort transaction request](./APIs/cloud-api-reference#abort-transaction) when attempting to cancel that specific payment request.
...
To enable printing receipts from the Sale System, include "PrinterReceipt" in the list of available [SaleTerminalData.SaleCapabilities](./data-dictionary#salecapabilities) when sending the [LoginRequest](./APIs/cloud-api-reference#login). 
...
If capable, the Sale System should present UI to the cashier for the duration of a payment. The content of the UI is set by the [input](./APIs/cloud-api-reference#input) and [display](./APIs/cloud-api-reference#display) request messages sent from the POI System.
...
To enable display and input handling on the Sale System, [SaleTerminalData.SaleCapabilities](./data-dictionary#salecapabilities) in the [login](./APIs/cloud-api-reference#login) request contains "CashierStatus", "CashierError", "CashierInput", and "CustomerAssistance".
...
The Sale System should immediately display an initial UI after sending a [payment](./APIs/cloud-api-reference#payment) request which informs the cashier the payment is in progress. This UI is required to handle instances where the first display/input message from the POI System is not present, or delayed.
...
The POI system will send zero or more [display request](./APIs/cloud-api-reference#display) messages to the Sale System during a transaction. The Sale System should display these messages and continue to wait for the payment response.
...
The POI system will send zero or more [input request](./APIs/cloud-api-reference#input) messages to the Sale System during a transaction. The Sale System should display these input requests, allow the cashier the option to answer the input request, and continue to wait for the payment response. The Sale System should not block on an input request, as the POI System may continue with the transaction before the Sale System has sent an input response.
...
This UI is constructed using the [Result](./data-dictionary#result), [ErrorCondition](./data-dictionary#errorcondition), and [AdditionalResponse](./data-dictionary#additionalresponse) fields of the `Response` object.
...
If the cashier initiates a payment cancellation, the Sale System sends an [abort transaction request](./APIs/cloud-api-reference#abort-transaction) (with `AbortRequest.AbortReason` field set to "User Cancel").  Then, the Sale System continues to wait for the payment response. 
...
- Cash out only (no sale items are included)
  ...
  - Set [PaymentTransaction.AmountsReq.RequestedAmount](./data-dictionary#requestedamount) to the cash out amount 
  - Set [PaymentTransaction.AmountsReq.CashBackAmount](./data-dictionary#cashbackamount) to the cash out amount 
- Purchase plus cash out (sale items are included)
  - Set [PaymentData.PaymentType](./data-dictionary#paymenttype) to "Normal"
  - Set [PaymentTransaction.AmountsReq.RequestedAmount](./data-dictionary#requestedamount) to the value of the sale items, plus the cash out amount 
  - Set [PaymentTransaction.AmountsReq.CashBackAmount](./data-dictionary#cashbackamount) to the cash out amount 
...
- Build a standard [payment request](./APIs/satellite-api-reference#payment)
...
Use the [Reconciliation](./APIs/cloud-api-reference#reconciliation) request to initiate a settlement.
...
To tokenise a card as part of a [payment](./APIs/cloud-api-reference#payment), set [SaleData.TokenRequestedType](./data-dictionary#tokenrequestedtype) in the payment request to "Transaction" or "Customer".
...
The [card acquisition](./APIs/cloud-api-reference#card-acquisition) request allows the Sale System to tokenise a card which can be used in future payment requests.
...
To perform a [payment](./APIs/cloud-api-reference#payment) request using a token: 
...
To perform a split payment, the Sale System should send multiple [payment](./APIs/cloud-api-reference#payment) requests, one for each split. 

Split is allowed when [PaymentType](./data-dictionary#paymenttype) is "Normal".

The following data elements should be set by the Sale System for a split payment in a [payment](./APIs/cloud-api-reference#payment) request:
...
- [PaymentTransaction.SaleItem](./data-dictionary#saleitem) array, which may reflect
  ...
  - All the items in the sale, in the case the sale is being split by amount. Note when splitting by amount the sum of the item amounts could more than the [RequestedAmount](./data-dictionary#requestedamount).
- [PaymentTransaction.AmountsReq.RequestedAmount](./data-dictionary#requestedamount) set to the amount of the split payment
- [PaymentData.SplitPaymentFlag] set to true
- [PaymentTransaction.AmountsReq.MinimumSplitAmount](./data-dictionary#minimumsplitamount) set to the minimum split amount 
- [PaymentTransaction.AmountsReq.PaidAmount](./data-dictionary#paidamount) set to the total amount of previous split payment associated with this sale
...
1. Sale System sends a [payment request](./APIs/cloud-api-reference#payment) request to the POI Terminal and sets
  - [PaymentTransaction.AmountsReq.RequestedAmount](./data-dictionary#requestedamount) to $110 (sale amount + tip)
  - [PaymentTransaction.AmountsReq.TipAmount](./data-dictionary#tipamount) to $10
...
1. Sale System sends a [payment request](./APIs/cloud-api-reference#payment) request to the POI Terminal and sets
  - [PaymentTransaction.AmountsReq.RequestedAmount](./data-dictionary#requestedamount) to $100 (sale amount + tip)
  - [PaymentTransaction.AmountsReq.TipAmount](./data-dictionary#tipamount) to $0
...
1. POI Terminal approves payment and returns the result to the Sale System with 
  - [PaymentResult.AmountsResp.AuthorizedAmount](./data-dictionary#authorizedamount) set to $110
  - [PaymentResult.AmountsResp.TipAmount](./data-dictionary#tipamount) set to $10
...
1. Cashier opens $100 sale on the Sale System and initiates payment
1. Sale System sends a [payment request](./APIs/cloud-api-reference#payment) request to the POI Terminal and sets [PaymentTransaction.AmountsReq.RequestedAmount](./data-dictionary#requestedamount) to $100
...
1. POI Terminal approves payment and returns the result to the Sale System with 
  ...
  - [PaymentResult.AmountsResp.AuthorizedAmount](./data-dictionary#authorizedamount) set to $101.90
  - [PaymentResult.AmountsResp.SurchargeAmount](./data-dictionary#surchargeamount) set to $1.90
...
When the Sale System sends a request, it will receive a matching response. For example, if the Sale System sends a [payment request](./APIs/cloud-api-reference#payment), it will receive a [payment response](./APIs/cloud-api-reference#payment-response).
...
More information about how to handle such scenario can be found in the [Error handling section of the Cloud API](./APIs/cloud-api-reference#error-handling)